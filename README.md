...
No manual download of the DINOv3 backbone weights is required: they are pulled automatically from `timm` on first use. The `model.pretrained_encoder` field in the YAML configuration is a legacy entry inherited from the CellViT pipeline and is ignored in this repository — it can be left as-is.

## 3. Dataset Preparation

We use the PanNuke dataset in the preprocessed form expected by the CellViT data loader, which this repository reuses without modification. The high-level steps are:
...
### 4.1 Paths and logging

| Field | Meaning |
|---|---|
| `data.dataset_path` | Path to the preprocessed PanNuke directory (Section 3). |
...
### 4.2 Switching the DINOv3 backbone
...
### 4.3 Other notable training options
