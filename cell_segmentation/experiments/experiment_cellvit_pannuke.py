...
class ExperimentCellVitPanNuke(BaseExperiment):
    ...
    def run_experiment(self) -> tuple[Path, dict, nn.Module, dict]:
        ...
        ### Machine Learning - Multi-GPU Support Added Here ###
        gpu_conf = self.run_conf['gpu']
        device_ids = []

        # Logic to handle single int, list of ints, or string "0,1"
        if isinstance(gpu_conf, list):
            device_ids = gpu_conf
            main_gpu = device_ids[0]
        elif isinstance(gpu_conf, str):
            if "," in gpu_conf:
                device_ids = [int(x) for x in gpu_conf.split(",")]
                main_gpu = device_ids[0]
            else:
                main_gpu = int(gpu_conf)
                device_ids = [main_gpu]
        else:
            # Assume int
            main_gpu = int(gpu_conf)
        ...
        self.logger.info(f"Using Main GPU: {device}")
        
        if len(device_ids) > 1:
            self.logger.info(f"Multi-GPU training enabled on devices: {device_ids}")
        else:
            self.logger.info(f"Single-GPU training on device: {device}")
        ...
        # Wrap with DataParallel if multiple GPUs are selected
        if len(device_ids) > 1:
    ...
    def get_loss_fn(self, loss_fn_settings: dict) -> dict:
        ...
        # --- REMOVED TISSUE TYPE LOSS BLOCK HERE ---
    ...
    def get_train_model(
        ...
        **kwargs,
        ...
        # check for backbones
        implemented_backbones = ["default", "vit256", "sam-b", "sam-l", "sam-h", "dinov3-s", "dinov3-b", "dinov3-l", "dinov3-h"]
        if backbone_type.lower() not in implemented_backbones:
            raise NotImplementedError(
                f"Unknown Backbone Type {backbone_type} - Currently supported are: {implemented_backbones}"
            )
        ...
        if backbone_type.lower() in ["dinov3-s", "dinov3-b", "dinov3-l", "dinov3-h"]:
            if shared_decoders:
            ...
            for name, param in model.named_parameters():
                if param.requires_grad:
                    self.logger.debug(f"Trainable parameter: {name}")
