...
class InferenceCellViT:
    ...
    def get_model(
        ...
        if model_type in ["CellViTDINOv3","CellViTDINOv3Shared"]:
            if model_type == "CellViTDINOv3":
                model_class = CellViTDINOv3
            elif model_type == "CellViTDINOv3Shared":
                model_class = CellViTDINOv3Shared
            model = model_class(
                model_path=None,
                num_nuclei_classes=self.run_conf["data"]["num_nuclei_classes"],
                num_tissue_classes=self.run_conf["data"]["num_tissue_classes"],
    ...
    def setup_patch_inference(
        ...
        # Handle DataParallel architecture name
        arch = checkpoint["arch"]
        if arch == "DataParallel":
            self.logger.info("Detected 'DataParallel' checkpoint architecture. Inferring actual architecture from configuration.")
            backbone = self.run_conf["model"].get("backbone", "default").lower()
            shared = self.run_conf["model"].get("shared_decoders", False)
            
            if backbone == "default":
                arch = "CellViTShared" if shared else "CellViT"
            elif backbone == "vit256":
                arch = "CellViT256Shared" if shared else "CellViT256"
            elif backbone in ["sam-b", "sam-l", "sam-h"]:
                arch = "CellViTSAMShared" if shared else "CellViTSAM"
            elif backbone in ["dinov3-s", "dinov3-b", "dinov3-l", "dinov3-h"]:
                arch = "CellViTDINOv3Shared" if shared else "CellViTDINOv3"
            else:
                self.logger.warning(f"Could not strictly infer architecture from backbone {backbone}. Defaulting to CellViT/Shared.")
                arch = "CellViTShared" if shared else "CellViT"
            self.logger.info(f"Inferred architecture: {arch}")

        model = self.get_model(model_type=arch)
        self.logger.info(
        ...
        if any(k.startswith("module.") for k in state_dict.keys()):
            self.logger.info("Detected 'module.' prefix in state_dict keys (from DataParallel). Removing prefixes.")
            new_state_dict = {}
    ...
    def run_patch_inference(
        ...
        # REMOVED: tissue_pred = []  # tissue predictions for each image
        ...
        dataset_metrics = {
            ...
            # REMOVED: "Tissue-Multiclass-Accuracy": tissue_detection_accuracy,
        ...
        # NEW: PanNuke Style Table (mPQ & bPQ)
        self.logger.info(f"{20*'*'} PanNuke Style Metrics (mPQ & bPQ) {20*'*'}")
        pannuke_table = []
        for key in tissue_metrics:
            pannuke_table.append(
                [
                    key.title(),
                    tissue_metrics[key]["mPQ"],
                    tissue_metrics[key]["bPQ"],
        ...
        self.logger.info(
            tabulate(
                pannuke_table, headers=["Tissue", "mPQ", "bPQ"], floatfmt=".4f", tablefmt="grid"
            )
        )
    ...
    def inference_step(
        ...
        # 提取 cosine_sim_map (如果在 predictions 中)
        cosine_sim_map = predictions.get("cosine_sim_map", None)
        ...
        if generate_plots:
            self.plot_results(
                ...
                scores=scores,
                cosine_sim_map=cosine_sim_map, # 传入 sim map
            )
    ...
    def unpack_predictions(
        ...
        """
        # REMOVED: predictions["tissue_types"]
    ...
    def calculate_step_metric(
        ...
        # REMOVED: preparation and device movement for tissue predictions
        ...
        # We need to iterate over the batch. Since we removed pred_tissue, we use gt length or batch_size
    ...
    def plot_results(
        ...
        cosine_sim_map: torch.Tensor = None,
    ) -> None:
        """Generate example plot with image, binary_pred, hv-map and instance map from prediction and ground-truth
           Saves results into separate folders per image ID.
        """
        outdir = Path(outdir)
        outdir.mkdir(exist_ok=True, parents=True)

        # 兼容 DataclassHVStorage 对象
        if hasattr(predictions, "get_dict"):
            predictions = predictions.get_dict()
        if hasattr(ground_truth, "get_dict"):
            ground_truth = ground_truth.get_dict()

        # Robustly determine H and W from input images (B, C, H, W)
        ...
        gt_hv = ground_truth["hv_map"]
        if gt_hv.shape[1] == 2 and gt_hv.ndim == 4:
             gt_sample_hv_map = gt_hv.permute(0, 2, 3, 1).detach().cpu().numpy()
        else:
             gt_sample_hv_map = gt_hv.detach().cpu().numpy()
        ...
        for i in range(len(img_names)):
            # 为每张图创建独立文件夹
            current_img_dir = outdir / str(img_names[i])
            current_img_dir.mkdir(exist_ok=True, parents=True)

            fig, axs = plt.subplots(figsize=(7, 2), dpi=300)
            placeholder = np.zeros((2 * h, 8 * w, 3))
            ...
            placeholder[:h, 4 * w : 5 * w, :3] = rgba2rgb(instance_map((gt_sample_instance_map[i] - np.min(gt_sample_instance_map[i])) / (np.max(gt_sample_instance_map[i]) - np.min(gt_sample_instance_map[i] + 1e-10))))
            placeholder[h : 2 * h, 4 * w : 5 * w, :3] = rgba2rgb(instance_map((pred_sample_instance_maps[i] - np.min(pred_sample_instance_maps[i])) / (np.max(pred_sample_instance_maps[i]) - np.min(pred_sample_instance_maps[i] + 1e-10))))
            # type_predictions
            ...
            # contours - GT
            gt_contours_polygon = [v["contour"] for v in ground_truth["instance_types"][i].values()]
            gt_contours_polygon = [list(zip(poly[:, 0], poly[:, 1])) for poly in gt_contours_polygon]
            gt_contour_colors_polygon = [cell_colors[v["type"]] for v in ground_truth["instance_types"][i].values()]
            gt_cell_image = Image.fromarray((sample_images[i] * 255).astype(np.uint8)).convert("RGB")
            gt_drawing = ImageDraw.Draw(gt_cell_image)
            add_patch = lambda poly, color: gt_drawing.line(poly, fill=color, width=2)
            [add_patch(poly, c) for poly, c in zip(gt_contours_polygon, gt_contour_colors_polygon)]
            
            # 保存 GT 到子文件夹
            gt_cell_image.save(current_img_dir / "raw_gt.png") 

            placeholder[:h, 6 * w : 7 * w, :3] = np.asarray(gt_cell_image) / 255
            
            # contours - Pred
            pred_contours_polygon = [v["contour"] for v in predictions["instance_types"][i].values()]
            pred_contours_polygon = [list(zip(poly[:, 0], poly[:, 1])) for poly in pred_contours_polygon]
            pred_contour_colors_polygon = [cell_colors[v["type"]] for v in predictions["instance_types"][i].values()]
            pred_cell_image = Image.fromarray((sample_images[i] * 255).astype(np.uint8)).convert("RGB")
            pred_drawing = ImageDraw.Draw(pred_cell_image)
            add_patch = lambda poly, color: pred_drawing.line(poly, fill=color, width=2)
            [add_patch(poly, c) for poly, c in zip(pred_contours_polygon, pred_contour_colors_polygon)]
            
            # 保存 Pred 到子文件夹
            pred_cell_image.save(current_img_dir / "raw_pred.png")

            placeholder[h : 2 * h, 6 * w : 7 * w, :3] = np.asarray(pred_cell_image) / 255

            # Cosine Similarity Map Visualization

            if cosine_sim_map is not None:
                # [B, 1, H, W] -> [H, W]
                sim_img = cosine_sim_map[i, 0].detach().cpu().numpy()
                
                # Invert the similarity map for visualization if needed
                # User report: nuclei are dark, background is bright.
                # Fix: 1 - sim_img to make nuclei bright.
                sim_img = 1.0 - sim_img

                # Enhance contrast: Min-Max Normalization + Power Law Transformation
                sim_min, sim_max = sim_img.min(), sim_img.max()
                if sim_max > sim_min:
                    # Stretch to [0, 1]
                    sim_img = (sim_img - sim_min) / (sim_max - sim_min)
                
                # Apply power to suppress background (gamma correction style)
                # Makes weak signals weaker and strong signals stronger
                sim_img = np.power(sim_img, 3)
                
                # 使用 magma colormap (通常用于 attention/saliency map)
                sim_colored = rgba2rgb(plt.get_cmap("jet")(sim_img))
                # print(sim_colored.shape)
                # placeholder[:h, 7 * w : 8 * w, :3] = sim_colored
                # placeholder[h : 2 * h, 7 * w : 8 * w, :3] = sim_colored
            ...
            #     # 保存 32x32 的 Similarity Map 到子文件夹
            #     sim_pil = Image.fromarray((sim_colored).astype(np.uint8))
            #     sim_32x32 = sim_pil.resize((256, 256), resample=Image.BILINEAR)
            #     sim_32x32.save(current_img_dir / "similarity_map_32x32.png")
            ...
            axs.set_xticks(np.arange(w / 2, 8 * w, w))
            axs.set_xticklabels(["Image","Binary-Cells","HV-Map-0","HV-Map-1","Instances","Nuclei-Pred","Countours","DINO-Sim"], fontsize=6)
            axs.xaxis.tick_top()
            axs.set_yticks(np.arange(h / 2, 2 * h, h))
            axs.set_yticklabels(["GT", "Pred."], fontsize=6)
            axs.tick_params(axis="both", which="both", length=0)
            grid_x = np.arange(w, 8 * w, w)
            ...
            # 保存组合大图到子文件夹，文件名为 overview.png
            fig.savefig(current_img_dir / "overview.png")
