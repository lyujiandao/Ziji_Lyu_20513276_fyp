...
#         return x[:, :, pad:-pad, pad:-pad]
    ...
    def forward(self, x, H, W):
        # pdb.set_trace()
        B, N, C = x.shape
        # print(f"shiftmlp input -B:{B}, N:{N}, C:{C}, H:{H}, W:{W}")
        # print(f"x.shape after dwconv and act: {x.shape}")
        # print(f"shift_size: {self.shift_size}, pad: {self.pad}")
        ...
        # actual_size = x.transpose(1, 2).numel()
        # if expected_size != actual_size:
        #     # 自动计算正确的 H 和 W
        #     actual_HW = actual_size // (B * C)
        #     actual_H = int(math.sqrt(actual_HW))
        #     actual_W = actual_HW // actual_H
